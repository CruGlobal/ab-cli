...
Command.help = function () {
  ...
  examples:
    ...
    $ appbuilder setup --port 8080 --tag master
        - sets .env var used by docker-compose.yml to listen on port 8080
        - sets .env var used by docker-compose.yml to use :master containers
...
Command.helpOptions = `
    ...
    --tag=[master,develop,latest] : <string> which version of the Docker containers to use.

    --platform=[docker,podman] : which platform to use ("docker" by default)

    authentication settings:
    --authType=[login,OKTA,CAS] : which authentication method to use for site users
    --casBaseURL=[string] : base url for cas
    --casEnabled=[boolean] : equivalent to --authType=toUpperCase
    --casUUIDKey=[string] : cas uuid key
    --oktaClientID=[string] : client id for Okta
    --oktaClientSecret=[string] : client secret for Okta
    --oktaDomain=[string] : domain for the Okta organization
    --oktaEnabled=[boolean] : equivalent to --authType=OKTA
    --siteURL=[string] : where the AppBuilder site will be hosted (for Auth redirect)

    settings for DB:
    --db.*     : options related to setting up the Maria DB environment
    --db.expose         : allow Maria DB to be accessed from your local network
    --db.port=[#]       : what port to listen on
    --db.password=[pwd] : the db root user password

    settings for relay:
    --relayEnabled=[boolean] : wether  to enable the relay service
    --relayServerToken=[string] : token to connect with the relay server
    --relayServerURL=[string] : url for the relay server
    --pwaURL=[string] : url for the pwa server
    `;
...
function defaultValues(done) {
   ...
   // use docker by default, podman the only other valid option
   if (Options.platform !== "podman") {
      Options.platform = "docker";
   }
