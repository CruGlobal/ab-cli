...
var pluginMigrateView = require(path.join(
   __dirname,
   "tasks",
   "pluginMigrateView.js"
));
// var pluginMigrateObject = require(path.join(__dirname, "tasks", "pluginMigrateObject.js"));
...
Command.help = function () {
  ...
  [operation]s :
    ...
    migrate :          $ appbuilder plugin migrate [View|Object|Model|Widget] [pluginName] [objectName]
...
Command.run = async function (options) {
   ...
   if (Options.operation === "migrate") {
      Options.objectType = options._.shift();
   }
   ...
   switch (Options.operation.toLowerCase()) {
      ...
      case "migrate":
         if (Options.objectType) {
            switch (Options.objectType.toLowerCase()) {
               case "view":
                  task = pluginMigrateView;
                  break;
               case "object":
                  task = pluginMigrateObject;
                  break;
            }
         }
         break;
