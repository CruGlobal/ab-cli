...
function createUpdateCommand(done) {
   ...
   // Also save this to ./test/setup
   fs.writeFileSync(`./test/setup/reset-user.sql`, contents);
...
async function updateCommand() {
   ...
   const cmd = `${platform} ps | awk '/${Options.stack}.db/ { print $1 }'`;
   const containerID = shell
      .exec(cmd, { silent: true })
      .stdout.replace("\n", "");

   // run the command
   let cmd2 = `${platform} exec -i ${containerID} mysql -uroot -p${Options.dbPassword} "appbuilder-admin" < cmd_tenant.sql`;
   // console.log(`command2[${cmd2}]`);
   await utils.logFormatter.exec(cmd2);
}
...
async function removeTempFiles() {
   await utils.logFormatter.exec("rm cmd_tenant.sql");
}
...
CYPRESS_USER_PASSWORD="${Options.password}"`,
         log: utils.logFormatter.format(
            "... patched: .env (CYPRESS VARIABLES)"
         ),
      },
   ];
