...
function createDirectory(done) {
   if (process.platform == "win32") {
      shell.exec(
         `${DockerCmdWindows} cordova create ${Options.appName} ${Options.appID} ${Options.appName}`
      );
   } else {
      shell.exec(
         `${DockerCmd} cordova create ${Options.appName} ${Options.appID} ${Options.appName}`
      );
   }
   done();
}
...
function installNodeModules(done) {
   ...
   if (Options.platforms.length > 0) {
      if (process.platform == "win32") {
         shell.exec(`${DockerCmdWindows} npm install ${nodeModules.join(" ")}`);
         shell.exec(
            `${DockerCmdWindows} npm install --save-dev ${nodeDevModules.join(
               " "
            )}`
         );
      } else {
         shell.exec(`${DockerCmd} npm install ${nodeModules.join(" ")}`);
         shell.exec(
            `${DockerCmd} npm install --save-dev ${nodeDevModules.join(" ")}`
         );
      }
   }
   ...
   var fileCopies = [
      {
         from: path.join(nodeDir, "es6-promise", "dist", "es6-promise.min.js"),
...
`);
         var err = new Error("ENOTFOUND: unknown directory : " + Options.dest);
         err._handled = true;
         done(err);
         return;
      }
...
function modifyFiles(done) {
   // update config options to work around QR
   //// NOTE: looks like cordova already includes these settings:
   var pluginFix = `<platform name="ios">
   ...
   utils.filePatch([
      {
         ...
         tag: /<platform\s*name="ios">/,
         replace: pluginFix,
         log: "step: fix QR scanner plugin config",
      },
      {
            ...
            .prompt([
               {
                  ...
                  default: `${Options.orgType}.${Options.orgName}.${Options.appName}`,
                  validate: (input) => {
                     ...
                     return input != ""
                        ? true
                        : `enter a valid entry like: com.yourOrg.${Options.appName}`;
                  },
                  when: (values) => {
                     return (
                        !values.appID && typeof Options.appID == "undefined"
                     );
                  },
               },
               {
                  ...
                  when: (values) => {
                     ...
                     return (
                        ((values.platforms &&
                           ...
                           (Options.platforms &&
                              Options.platforms.indexOf("ios") > -1)) &&
                  ...
                  when: (values) => {
                     return (
                        ((values.platforms &&
                           ...
                           (Options.platforms &&
                              Options.platforms.indexOf("android") > -1)) &&
