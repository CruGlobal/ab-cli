 ...
 * @returns {Promise}
 *    Options.pluginDir = path to plugin directory
 *    Options.pluginDirName = plugin directory name
 */
 ...
 * @param {string} pluginType - name of platform for user messages (e.g., "service", "web")
 * @returns {Promise}
 */
function questions(Options, pluginType) {
   return new Promise((resolve, reject) => {
      ...
      function processObjectName() {
         ...
         Options.pluginKey = `ab-${pluginType}-${pluginKey}`;
      }
      ...
      if (!Options.type) {
         prompts.push({
            name: "type",
            type: "input",
            message: `Enter the type for the ${pluginType} (e.g., 'object', 'model'):`,
            default: "object",
            validate: (input) => {
               if (!input || input.trim() === "") {
      ...
      if (Options.objectName === Options.pluginName) {
         prompts.push({
            name: "objectName",
            type: "input",
            message: `Enter the object name for the ${pluginType} (e.g., 'ObjectNetsuite'):`,
