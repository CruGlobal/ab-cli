...
function generateTestConfigs(done) {
   console.log(`Configure test volumes`);
   utils
      .configInit({
         dbVolume: "keep",
         stack: Options.stack,
...
function waitClosed(done) {
   var stack = shell
      ...
      .grep(Options.stack)
      .stdout.replace(/\n*\r*/g, "");
   //Need to wait for the network to be removed or waitBoot will fail.
   var network = shell
      .exec(`docker network ls`, { silent: true })
      .grep(Options.stack)
      .stdout.replace(/\n*\r*/g, "");
   if (stack != "" || network != "") {
      // stack is found so:
...
/**
 * Replace the test values in .env for cypress
 */
async function writeEnvironment() {
   utils.writeEnvironment({
      CYPRESS_STACK: Options.stack,
