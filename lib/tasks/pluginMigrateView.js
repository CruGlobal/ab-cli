...
Command.help = function () {
  ...
  usage: $ appbuilder plugin migrate view [viewName] [options]
...
Command.run = async function (options) {
   ...
   Options.pluginName = "migrate_view_" + nanoid(6);
   Options.pluginName = Options.pluginName.toLowerCase();
   ...
   //// Now remove the intermediary placeholder plugin/files
   await removeIntermediaryFiles(Options);
};
...
function migrateEditor(editorPath) {
   return new Promise((resolve, reject) => {
      try {
         ...
         // Within constructor(): remove this.AB/view/component lines;
         // change super(base, { to super(view, base, {
         const constructorMatch = classEditorContents.match(
            /constructor\s*\([^)]*\)\s*\{/
         );
         if (constructorMatch) {
            const constructorStart = classEditorContents.indexOf(
               constructorMatch[0]
            );
            const bodyStart = constructorStart + constructorMatch[0].length;
            let depth = 1;
            let pos = bodyStart;
            while (pos < classEditorContents.length && depth > 0) {
               const ch = classEditorContents[pos];
               if (ch === "{") depth += 1;
               else if (ch === "}") depth -= 1;
               pos += 1;
            }
            const bodyEnd = pos - 1; // position of closing }
            let constructorBody = classEditorContents.slice(bodyStart, bodyEnd);
            constructorBody = constructorBody
               .replace(/\n\s*this\.AB\s*=[^\n]*/g, "")
               .replace(/\n\s*this\.view\s*=[^\n]*/g, "")
               .replace(/\n\s*this\.component\s*=[^\n]*/g, "")
               .replace(/\bsuper\s*\(\s*(?!view\s*,)base/g, "super(view, base");
            classEditorContents =
               classEditorContents.slice(0, bodyStart) +
               constructorBody +
               classEditorContents.slice(bodyEnd);
         }
   ...
   return new Promise((resolve, reject) => {
      try {
         ...
         let matchPluginKey = contentsWebPlugin.match(
            /ABView\w*Defaults\s*=\s*\{[\s\S]*?key:\s*"([^"]+)"/
         );
...
/**
 * @function removeIntermediaryFiles
 * Remove the intermediary plugin/[directory] and included files.
 * @param {*} Options
 * @returns {Promise}
 */
function removeIntermediaryFiles(Options) {
   return new Promise((resolve, reject) => {
      try {
