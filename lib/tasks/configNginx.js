...
`;
   ...
   // package the docker-compose.override.yml entry for nginx
   var overrideContents = utils.fileRender(
      path.join(utils.fileTemplatePath(), "_nginx.dockercompose.override.yml"),
      Options
   );
   var overrideEntry = `services:
...
`;
  ...
  redis_data:`;
   ...
   // The config-compose.yml needs to now mount and create our nginx
   // volume settings
      ...
      patches = patches.concat([
         {
            ...
            tag: "services:",
            replace: overrideEntry,
            log: "nginx: adding config to docker-compose.override.yml",
         },
         {
            file: path.join(process.cwd(), "docker-compose.yml"),
            tag: "redis_data:",
